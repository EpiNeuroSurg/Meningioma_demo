...
void compute_bilateral_kernel(float* const output_kernel,
                              const Tensor& rgb_tensor, const float theta_alpha,
  ...
  for (int p = 0; p < num_pixels; ++p) {
    ...
    output_kernel[5 * p + 3] =
        static_cast<float>(rgb(num_pixels + p) / theta_beta);
    output_kernel[5 * p + 4] =
        static_cast<float>(rgb(2 * num_pixels + p) / theta_beta);
  }
}
...
class HighDimFilterOp : public OpKernel {
 public:
  explicit HighDimFilterOp(OpKernelConstruction* context) : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("bilateral", &bilateral_));
    OP_REQUIRES_OK(context, context->GetAttr("theta_alpha", &theta_alpha_));
    OP_REQUIRES_OK(context, context->GetAttr("theta_beta", &theta_beta_));
    OP_REQUIRES_OK(context, context->GetAttr("theta_gamma", &theta_gamma_));
    OP_REQUIRES_OK(context, context->GetAttr("backwards", &backwards_));
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (bilateral_) {
      ...
      compute_bilateral_kernel(kernel_vals, image_tensor, theta_alpha_,
                               theta_beta_);
...
REGISTER_KERNEL_BUILDER(Name("HighDimFilter").Device(DEVICE_CPU),
                        HighDimFilterOp);
