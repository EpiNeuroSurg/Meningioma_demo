...
void ModifiedPermutohedral::init(const float* features, int num_dimensions,
                                 int num_points) {
  ...
  for (int k = 0; k < N_; k += blocksize) {
    ...
    for (int j = 0; j < d_; j++)
      for (int i = 0; i < blocksize; i++)
        ff[j * blocksize + i] =
            k + i < N_ ? *(features + (k + i) * num_dimensions + j) : 0.0;
...
#endif
void ModifiedPermutohedral::seqCompute(Tensor& out_tensor,
                                       const Tensor& in_tensor, int value_size,
...
#ifdef SSE_PERMUTOHEDRAL
void ModifiedPermutohedral::sseCompute(Tensor& out_tensor,
                                       const Tensor& in_tensor, int value_size,
                                       const bool reverse,
...
#else
void ModifiedPermutohedral::sseCompute(Tensor& out, const Tensor& in,
...
void ModifiedPermutohedral::compute(Tensor& out, const Tensor& in,
                                    int value_size, bool reverse,
                                    bool add) const {
  if (value_size <= 2)
    seqCompute(out, in, value_size, reverse, add);
